...
export async function githubLogin() {
  ...
  window.location.assign(`${API_BASE_URL}/auth/github?source=web`);
}
...
export async function getProfiles(url?: string) {
  try {
    ...
    console.log('Fetching profiles from:', fetchUrl);
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Failed to fetch profiles: ${response.status} - ${errorText}`
      );
    }
...
export async function exportProfiles(filters?: {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Failed to export profiles: ${response.status} - ${errorText}`
      );
    }
...
async function apiFetch(input: RequestInfo, init?: RequestInit) {
  ...
  if (!response.ok) {
    ...
    const shouldRefresh =
      response.status === 401 ||
      (response.status === 403 &&
        /invalid token|invalid or expired token/i.test(errorBody));
